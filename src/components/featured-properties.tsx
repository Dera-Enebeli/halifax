  ...
  return (
    <div className="group bg-white rounded-xl overflow-hidden shadow-sm border border-warm-border transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5">
      <div className="relative h-52 overflow-hidden">
        ...
        />
        <div className="absolute top-3 left-3 bg-near-black text-white text-[11px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full">
          {property.status === "Active" ? (property.type === "Condo" ? "Rent" : "Sale") : property.status}
        </div>
      </div>

      <div className="p-5">
        <h3 className="text-[17px] font-bold text-near-black mb-1 leading-snug">
          {property.title}
        </h3>
        ...
        <div className="flex items-center gap-4 mb-4 pb-4 border-b border-warm-border">
          ...
          <div className="flex items-center gap-1.5 text-gray-500">
            ...
            <span className="text-[13px] font-medium">{property.garage}</span>
          </div>
          <div className="flex items-center gap-1.5 text-gray-500">
            <Square className="h-4 w-4" />
            <span className="text-[13px] font-medium">{property.sqft}</span>
          </div>
        </div>

        <div className="flex items-center justify-between">
          <span className="text-[18px] font-bold text-near-black">
            {formatter.format(property.price)}
          </span>
          <Link
            href={`/listings/${property.id}`}
            className="text-[13px] font-semibold text-white bg-terracotta hover:bg-terracotta-dark px-4 py-2 rounded-full transition-all duration-200"
          >
            View Property
          </Link>
        </div>
